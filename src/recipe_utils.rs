...
/// rule_workload + extend
pub fn run_workload_extend<L: SynthLanguage>(
  workload: Workload,
  prior: Ruleset<L>,
  prior_limits: Limits,
  minimize_limits: Limits,
  ...
  let mut new = run_workload_internal(
    workload,
    prior.clone(),
    prior_limits,
    minimize_limits,
...
pub fn run_fast_forwarding<L: SynthLanguage>(
  ...
  let (allowed, _) =
    prior.partition(|rule| L::is_allowed_rewrite(&rule.lhs, &rule.rhs));
  ...
  let eg_denote =
    Scheduler::Simple(prior_limits).run(&eg_allowed, &exploratory);
...
/// Incrementally construct a ruleset by running rule inference up to a size
/// bound, using previously-learned rules at each step.
...
/// Util function for making a grammar with variables, values, and operators
...
mod test {
  ...
  #[test]
  fn iter_metric_fast() {
    // This test will not finish if the pushing monotonic filters through plugs
  ...
  #[test]
  fn empty_plug() {
    let wkld = iter_metric(base_lang(3), "EXPR", Metric::Atoms, 6)
      .plug("OP3", &Workload::empty());
