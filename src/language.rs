...
use egg::{
  Analysis, AstSize, CostFunction, DidMerge, ENodeOrVar, FromOp, Language,
  PatternAst, RecExpr,
};
...
pub struct Signature<L: SynthLanguage> {
  pub fuzz_constant: Option<L::Constant>,
  pub cvec: CVec<L>,
...
impl<L: SynthLanguage> Signature<L> {
  ...
  pub fn fuzz_constant(&self) -> Option<L::Constant> {
    self.fuzz_constant.clone()
  }
  pub fn update_fuzz_constant(&mut self) {
    self.fuzz_constant = self.get_fuzz_constant()
  }
  pub fn with_fuzz_constant(self) -> Self {
    let fuzz_constant = self.get_fuzz_constant();
    Self {
      fuzz_constant,
      ..self
    }
  }
  pub fn get_fuzz_constant(&self) -> Option<L::Constant> {
    let fuzz_constant = if self.cvec.is_empty() {
      None
    } else {
      if let Some(first_constant) = self
        .cvec
        .iter()
        .find(|v| v.is_some())
        .map(|x| x.clone().unwrap())
      {
        if self.cvec.iter().all(|v| {
          if let Some(constant) = v {
    ...
    let simplest = if enode.is_var() || enode.is_constant() {
      ...
      enode.for_each(|id| {
        if map.get(&id).is_none() {
          ...
          for n in s.as_ref() {
            nodes
              .push(n.clone().map_children(|id| Id::from(usize::from(id) + i)));
          }
    ...
    if !to.cvec.is_empty() && !from.cvec.is_empty() {
      for i in 0..to.cvec.len() {
        match (to.cvec[i].clone(), from.cvec[i].clone()) {
          ...
          (Some(x), Some(y)) => {
            if x != y {
              println!("to is {}", to.simplest.to_string());
              println!("from is {}", from.simplest.to_string());
            }
            assert_eq!(
              x, y,
              "cvecs do not match!! there could be wrong rewrites. consider add more constants when `initialize_vars`."
            )
          }
          _ => (),
        }
...
pub trait SynthLanguage:
  Language + Send + Sync + Display + FromOp + 'static
{
  /// Domain value type
  ...
  ///
  /// Most domains won't need to use the egraph parameter to add a constant
  /// node. However, Nat represents numbers recursively, so adding a new
  /// constant requires adding multiple nodes to the egraph.
  fn mk_constant(
    c: Self::Constant,
  ...
  /// finding candidates.
  /// If fast-forwarding is enabled, L::get_exploratory_rules() and
  /// L::is_allowed_op() must be implemented
